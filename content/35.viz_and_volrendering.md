...
As `yt` has expanded to support both patch-based AMR hierarchies and octree-organized datasets, volume rendering has necessarily adopted different traversal strategies for these different organizations of data.
For patch-based datasets, overlapping grids must be reconciled so that only the highest-resolution cells are sampled, and that they are sampled in the correct order along the ray.
For octree datasets, the recursive spatial hierarchy can instead be exploited directly.
Accordingly, `yt` provides distinct traversal algorithms for these classes of dataset while preserving a common interface for the sampling operations themselves.
...
Conceptually, it is easiest to think about this as constructing a global "ordering" of the cells, and then an in-order application of the sampler function with affiliated ray-specific data storage.
